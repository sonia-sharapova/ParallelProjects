...
type RubricTest struct {
	...
	Timeout     string `json:"timeout"`     // Overall timeout for the test
...
func runTests(rubric Rubric, gradescopePtr *bool, gradescopeVisPtr *string) {
	...
	for iIdx, item := range rubric.Items {
		for _, test := range item.Tests {
			...
			cmd := exec.Command("go", "test", test.PackageDir, "-run", test.Name, "-json", "-count=1", "-timeout", test.Timeout)
			...
			if spawnTest {
				...
				go func() {
					...
					for {
						...
						if test.TableDriven && testEvent.Test == fmt.Sprintf("Test%s", test.Name) {
							continue
						}
